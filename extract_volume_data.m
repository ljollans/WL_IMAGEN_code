function [meanvoldata,stdvoldata,sumvoldata,mask_name,volume_name] = extract_volume_data(masks2use,volfiles,where2save,filename4saving,volthreshupr,volthreshlwr,nanthreshold,hdrinfo2exclude)
...
%     mask2extractfrom = spm_vol(masks2use{masklpr});
    ...
    for vollpr=1:size(volfiles,2)
        ...
        if sum(isnan(voldata_in_mask(:)))<nanthreshold && isempty(strfind(vol2extractfrom.descrip,hdrinfo2exclude))==1
            meanvoldata(masklpr,vollpr) = nanmean(voldata(voldata_in_mask));
            stdvoldata(masklpr,vollpr) = nanstd(voldata(voldata_in_mask)); %std replaced by nanstd by Emily Jollans 19-08-14
            sumvoldata(masklpr,vollpr)=nansum(voldata(voldata_in_mask));%sum replaced by nansum by Emily Jollans 19-08-14
            volume_name{masklpr,vollpr}={volfiles{vollpr}};%keep a record of the volume name in case needed again
        end
        catch
...
ok2proceed=1;
checkvol=spm_vol(volfiles);%**ASSUME** all vols have the same dimension, so only need to check the 1st one
checkmask=spm_vol(masks2use);%**ASSUME** all masks have the same dimension, so only need to check the 1st one
